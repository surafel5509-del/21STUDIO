...
- If using web research, synthesize findings, distinguish facts from uncertainty, and include a concise Sources section with the exact URLs returned by the search tool.
...
def _system_message(mode: str, thinking_mode: bool) -> dict[str, str]:
    mode_instruction = {
        ...
        "agent": "Mode: Agent. Plan the task, use available tools when useful, then deliver a concrete result.",
        "research": "Mode: Research. MUST use the web_search tool for current, source-backed or research requests. Search, compare multiple results when possible, synthesize them, and include exact source URLs in a Sources section.",
    }.get(mode, "Mode: Chat.")
    thinking_instruction = ("Thinking mode is enabled: take extra care with planning and verification, but NEVER reveal private chain-of-thought. Return only a concise reasoning summary and the final result." if thinking_mode else "Thinking mode is off: answer directly while still checking your work.")
...
class Agent:
    ...
    async def stream(self, message: str, requested_provider: str = "auto", conversation_id: str | None = None, mode: str = "chat", thinking_mode: bool = False) -> AsyncIterator[tuple[str, str, str]]:
        ...
        # Research and Agent modes use the full tool loop so web search/calculation actually run.
